...
1. **COMPLETE STANDALONE STORY**: Each clip MUST tell a complete mini-story or teach a complete idea. The viewer should understand the clip WITHOUT needing any outside context. If someone watches ONLY this 20-90 second clip, they should walk away having learned something specific or felt something powerful.
...
❌ DO NOT select clips shorter than 20 seconds or longer than 90 seconds — they are not suitable for vertical short-form algorithms
...
[
  {
    ...
    "social_caption": "An engaging TikTok/Shorts caption beginning with a strong hook sentence, a summary bullet point, and 3-5 trending hashtags (e.g., #focus #mindset #motivation). Keep it clean, punchy, and ready to paste.",
    "viral_rating": 8.8,
    "retention_score": 9.0,
    "viral_analysis": "One short sentence analyzing the visual hook strength, audio pacing, and retention value of this segment.",
    "broll_cues": [
      {"keyword": "brain neurons firing", "offset_s": 5.0},
      {"keyword": "person meditating sunrise", "offset_s": 18.0},
      {"keyword": "gym workout motivation", "offset_s": 32.0}
    ],
...
## B-ROLL CUE GUIDELINES:
- broll_cues should contain 1-3 moments where a stock footage overlay would enhance the visual storytelling
- offset_s is the number of seconds FROM THE START OF THE CLIP (not the original video) where the B-Roll should appear
- keyword should be a 2-4 word search term for stock footage (e.g. "city skyline night", "brain neural network", "morning routine coffee")
- Place B-Roll during explanatory or metaphorical moments, NOT during direct-to-camera speech
- Space B-Roll cues at least 6 seconds apart to avoid visual clutter
- Use vivid, specific search terms — "ocean waves sunset" is better than "nature"
...
def detect_hooks(
    ...
    smartest_models = [
        "meta/llama-3.3-70b-instruct",          # Primary: Llama 3.3 70B (fast, high rate limits)
        "qwen/qwen3.5-122b-a10b",              # Fallback 1: Qwen3.5 122B
        "qwen/qwen3.5-397b-a17b",              # Fallback 2: Qwen3.5 397B
    ]
    ...
    for m in smartest_models:
        try:
            ...
            completion = client.chat.completions.create(
                ...
                top_p=0.85,
                timeout=180.0          # 180-second timeout to allow full-transcript digesting without premature failovers
            )
