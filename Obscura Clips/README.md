...
> Built for Windows · Ryzen 7 + RTX 3050 4GB · GPU-Accelerated & Thermally Optimized
...
- 🧠 **Smart Hook Detection** — **Qwen 3.5 397B** via NVIDIA NIM cloud API (finds actual 10/10 standalone stories)
- 👁️ **Face-Tracked Cropping** — MediaPipe (keeps the speaker centered in the 9:16 crop)
...
| Hook Detection | **☁ NVIDIA NIM** | Qwen 3.5 397B (Titan MoE) runs in the cloud to select premium clips |
...
Simply double-click **`Start Obscura Clips.bat`**. The script will automatically:
...
| `--music` | `auto` | Music vibe: `none`, `auto`, `ambient`, `lofi`, `focus` |
...
```
Obscura Clips/
├── Start Obscura Clips.bat  ← Double click to run the app
...
│   ├── hook_detector.py     ← Stage 3: Qwen 3.5 NIM Hook Detection
│   ├── face_tracker.py      ← Stage 4: MediaPipe face tracking
...
## GitHub Release Instructions (For Uploading)

When uploading this project to GitHub as a ZIP release, **delete these folders first** to keep the ZIP file small (under 10MB instead of multiple gigabytes):
1. `venv/` (Contains heavy packages, libraries, and CUDA DLLs which the `.bat` file will recreate on run).
2. `temp/` (Contains video/audio caches).
3. `output/` (Contains your rendered clips).
