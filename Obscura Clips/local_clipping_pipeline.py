...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    logger.info("═══ STAGE 2/6 ─ ASR Transcription (CPU INT8) ═══════")
    from modules.transcriber import (
        transcribe_audio,
        words_to_timed_transcript
    )
    import json

    words_cache_path = os.path.join(temp_dir, "words.json")
    if os.path.exists(words_cache_path):
        logger.info(f"   [Transcriber] Found cached transcription words at {words_cache_path}. Skipping transcription!")
